...
def convert_masks_to_yolo(masks_folder, labels_folder, class_id=0, min_area=10):
    ...
    for mask_file in masks_path.iterdir():
        if mask_file.suffix.lower() in supported_extensions:
            ...
            label_filename = mask_file.stem.replace("_mask", "_original")
...
if __name__ == "__main__":
    ...
    image_name = mask_name.replace("_mask", "_original")
    print(image_name)
    image_file = os.path.join(images_folder, f"{image_name}.tif")
    label_file = os.path.join(labels_folder, f"{image_name}.txt")

    visualize_yolo_labels(image_file, mask_file, label_file,f"./visualization_{image_name}.png")
